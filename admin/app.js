...
function contractCountrySelect(field, label, value, locked) {
  var countries = state.countries.map(function(country) { return country.country_name; });
  if (value && countries.indexOf(value) === -1) countries.unshift(value);
  return contractSelect('travel', field, label, value, countries, locked, 'contract-country');
}
...
function contractEditorHtml(contract) {
  ...
  html += contractSection('Carrier Details', '<div class="form-row">' + ['representative_name','representative_signature','office_address','email','website','office_phone','cell_phone'].map(function(field) { var labels={representative_name:'Representative Name',representative_signature:'Representative Signature / Name',office_address:'Office Address',email:'Email',website:'Website',office_phone:'Office Phone',cell_phone:'Cell Phone'}; return contractInput('carrier',field,labels[field],contractValue(data,'carrier',field),field === 'email' ? 'email' : 'text',locked); }).join('') + '</div>');
...
async function newContract() { openModal('New Contract', contractEditorHtml()); calculateAdminContractTotals(); await populateContractQuotes(); await populateContractCountries(); }
async function editContract(id) { var c=state.contracts.find(function(x){return x.id===id;}); if(!c)return; openModal((c.status==='signed'?'Signed':'Edit')+' Contract',contractEditorHtml(c)); calculateAdminContractTotals(); await populateContractQuotes(c.quote_request_id); await populateContractCountries(); }
...
async function populateContractCountries() { try { var d=await (await fetch('/api/countries')).json(); state.countries=d.countries||[]; document.querySelectorAll('.contract-country').forEach(function(select) { var selected=select.value; Array.from(select.options).forEach(function(option) { if (option.value) option.remove(); }); if (selected && !state.countries.some(function(country) { return country.country_name === selected; })) { var savedOption=document.createElement('option'); savedOption.value=selected; savedOption.textContent=selected; savedOption.selected=true; select.appendChild(savedOption); } state.countries.forEach(function(country) { var option=document.createElement('option'); option.value=country.country_name; option.textContent=country.country_name; option.selected=country.country_name===selected; select.appendChild(option); }); }); } catch(e){console.error(e);} }
async function importQuoteToContract(id) { if(!id)return; var r=await fetch('/api/admin/contracts/quotes/'+id,creds),d=await r.json();if(!r.ok){showToast(d.error||'Could not import quote','error');return;} document.getElementById('modalBody').innerHTML=contractEditorHtml({contract_data:d.contract_data,quote_request_id:id,status:'draft',id:document.getElementById('fContractId').value}); calculateAdminContractTotals(); await populateContractQuotes(id); await populateContractCountries(); }
