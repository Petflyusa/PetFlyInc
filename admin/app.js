...
async function saveEditedPetConnectMember(id) { var payload={first_name:document.getElementById('pmFirst').value,last_name:document.getElementById('pmLast').value,email:document.getElementById('pmEmail').value,phone:document.getElementById('pmPhone').value,city:document.getElementById('pmCity').value,state:document.getElementById('pmState').value,country:document.getElementById('pmCountry').value,postal_code:document.getElementById('pmPostal').value,email_alert_radius:document.getElementById('pmRadius').value,email_alerts:document.getElementById('pmAlerts').checked}; var r=await fetch('/api/admin/petconnect/members/'+id,{...creds,method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}),d=await r.json(); if(!r.ok){showToast(d.error||'Could not update member','error');return;} closeModal(); loadPetConnect(); showToast('Member updated','success'); }
