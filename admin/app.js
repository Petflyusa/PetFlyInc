...
function showSection(name) {
  ...
  if (name === 'petconnect-alerts') loadPetConnectAlerts();
  if (name === 'petconnect-partners') loadPetConnectPartners();
...
async function pcJson(url) { var r=await fetch(url,creds); if(!r.ok)throw new Error('Could not load PetConnect data'); return r.json(); }
async function loadPetConnectOverview() { try { var d=await pcJson('/api/admin/petconnect/summary'); document.getElementById('pcSummaryMembers').textContent=d.members.total||0; document.getElementById('pcSummaryMembersMeta').textContent=(d.members.verified||0)+' verified'; document.getElementById('pcSummaryPets').textContent=d.pets.total||0; document.getElementById('pcSummaryPetsMeta').textContent=(d.pets.missing||0)+' missing'; document.getElementById('pcSummaryAlerts').textContent=d.alerts.total||0; document.getElementById('pcSummaryAlertsMeta').textContent=(d.alerts.active||0)+' active'; document.getElementById('pcSummaryPartners').textContent=d.partners.total||0; document.getElementById('pcSummaryPartnersMeta').textContent=(d.partners.active||0)+' active'; loadEmailHealth(); } catch(e){showToast(e.message,'error');} }
async function loadPetConnectMembers() { try { var q=new URLSearchParams({search:pcValue('pcMemberSearch'),verified:pcValue('pcMemberVerified'),country:pcValue('pcMemberCountry'),alerts:pcValue('pcMemberAlerts')}); var d=await pcJson('/api/admin/petconnect/members?'+q); state.petconnect.members=d.members||[]; renderPetConnectMembers(); } catch(e){showToast(e.message,'error');} }
function renderPetConnectMembers() { pcTable('petconnectMembers',['Member','Address','Verified','Email alerts','Actions'],state.petconnect.members.map(function(m){return '<tr><td><strong>'+escHtml(m.first_name+' '+m.last_name)+'</strong><br>'+escHtml(m.email)+'</td><td>'+escHtml([m.address_line,m.city,m.state,m.postal_code,m.country].filter(Boolean).join(', ')||'—')+'</td><td>'+(m.is_verified?'Yes':'No')+'</td><td>'+(m.email_alerts?'On':'Off')+' · '+escHtml(m.email_alert_radius||100)+' mi</td><td class="col-actions"><button class="btn-outline" onclick="editPetConnectMember('+m.id+')" title="Edit"><i class="fas fa-edit"></i></button> <button class="btn-danger" onclick="deletePetConnectMember('+m.id+')"><i class="fas fa-trash"></i></button></td></tr>'; })); }
async function loadPetConnectPets() { try { var q=new URLSearchParams({search:pcValue('pcPetSearch'),species:pcValue('pcPetSpecies'),missing:pcValue('pcPetMissing')}); var d=await pcJson('/api/admin/petconnect/pets?'+q); state.petconnect.pets=d.pets||[]; pcTable('petconnectPets',['Pet','Microchip','Owner','Missing'],state.petconnect.pets.map(function(p){return '<tr><td><strong>'+escHtml(p.pet_name)+'</strong><br>'+escHtml([p.species,p.breed,p.color].filter(Boolean).join(' · '))+'</td><td>'+escHtml(p.microchip_number||'—')+'</td><td>'+escHtml(p.member_name)+'<br>'+escHtml(p.member_email)+'</td><td>'+(p.is_missing?'Yes':'No')+'</td></tr>'; })); } catch(e){showToast(e.message,'error');} }
async function loadPetConnectAlerts() { try { var q=new URLSearchParams({search:pcValue('pcAlertSearch'),alert_type:pcValue('pcAlertType'),status:pcValue('pcAlertStatus'),country:pcValue('pcAlertCountry')}); var d=await pcJson('/api/admin/petconnect/alerts?'+q); state.petconnect.alerts=d.alerts||[]; pcTable('petconnectAlerts',['Pet','Type / Location','Status','Actions'],state.petconnect.alerts.map(function(a){return '<tr><td><strong>'+escHtml(a.pet_name)+'</strong><br>'+escHtml(a.member_email)+'</td><td>'+escHtml(a.alert_type)+'<br>'+escHtml([a.last_seen_city,a.last_seen_state].filter(Boolean).join(', '))+'</td><td><select onchange="savePetConnectAlert('+a.id+',this.value)"><option '+(a.status==='active'?'selected':'')+'>active</option><option '+(a.status==='found'?'selected':'')+'>found</option><option '+(a.status==='closed'?'selected':'')+'>closed</option></select></td><td><button class="btn-danger" onclick="deletePetConnectAlert('+a.id+')"><i class="fas fa-trash"></i></button></td></tr>'; })); } catch(e){showToast(e.message,'error');} }
function filterPetConnectPartners(){state.petconnect.partnerPagination.page=1;state.petconnect.partnerSelection={};loadPetConnectPartners();}
function setPetConnectPartnerPage(page){state.petconnect.partnerPagination.page=page;loadPetConnectPartners();}
function setPetConnectPartnerPerPage(){state.petconnect.partnerPagination.page=1;state.petconnect.partnerSelection={};loadPetConnectPartners();}
function togglePetConnectPartnerSelection(id,checked){if(checked)state.petconnect.partnerSelection[id]=true;else delete state.petconnect.partnerSelection[id];updatePartnerSelectionStatus();}
function toggleAllPetConnectPartners(checked){state.petconnect.partners.forEach(function(partner){if(checked)state.petconnect.partnerSelection[partner.id]=true;else delete state.petconnect.partnerSelection[partner.id];});loadPetConnectPartners();}
function selectedPetConnectPartnerIds(){return Object.keys(state.petconnect.partnerSelection).map(Number).filter(Number.isInteger);}
function updatePartnerSelectionStatus(){var el=document.getElementById('partnerSelectionStatus');if(el)el.textContent=selectedPetConnectPartnerIds().length+' selected';}
function pcPartnerGeo(partner){if(partner.geocode_status==='located'&&partner.latitude!==null&&partner.longitude!==null)return '<span class="pc-geo pc-geo-located"><i class="fas fa-location-dot"></i> '+Number(partner.latitude).toFixed(5)+', '+Number(partner.longitude).toFixed(5)+'</span><small>'+escHtml(String(partner.geocoded_at||'').slice(0,10))+'</small>';var label=partner.geocode_status==='needs_review'?'Needs review':partner.geocode_status==='failed'?'Failed':'Pending';var retry=(partner.geocode_status==='needs_review'||partner.geocode_status==='failed')?' <button class="btn-outline pc-geo-retry" onclick="retryPartnerGeocoding('+partner.id+')" title="Retry geocoding"><i class="fas fa-rotate-right"></i></button>':'';return '<span class="pc-geo pc-geo-'+escHtml(partner.geocode_status||'pending')+'" title="'+escHtml(partner.geocode_error||'Address is waiting to be geocoded.')+'">'+label+'</span>'+retry;}
function renderPetConnectPartnerPagination(){var box=document.getElementById('partnerPagination'),p=state.petconnect.partnerPagination;if(!box)return;box.innerHTML='<span>'+p.total+' matching organizations</span><div><button class="btn-outline" '+(p.page<=1?'disabled':'')+' onclick="setPetConnectPartnerPage('+(p.page-1)+')"><i class="fas fa-chevron-left"></i></button><span>Page '+p.page+' of '+p.pages+'</span><button class="btn-outline" '+(p.page>=p.pages?'disabled':'')+' onclick="setPetConnectPartnerPage('+(p.page+1)+')"><i class="fas fa-chevron-right"></i></button></div>';}
async function loadPetConnectPartners() { try { var p=state.petconnect.partnerPagination,q=new URLSearchParams({search:pcValue('pcPartnerSearch'),partner_type_id:pcValue('pcPartnerType'),active:pcValue('pcPartnerActive'),verified:pcValue('pcPartnerVerified'),country:pcValue('pcPartnerCountry'),invite:pcValue('pcPartnerInvite'),geocode_status:pcValue('pcPartnerGeoStatus'),page:p.page,per_page:pcValue('pcPartnerPerPage')||'50'}); var d=await pcJson('/api/admin/petconnect/partners?'+q); state.petconnect.partners=d.partners||[];state.petconnect.partnerPagination=d.pagination||p;if(!state.petconnect.types.length){var t=await pcJson('/api/admin/petconnect/partner-types');state.petconnect.types=t.types||[];var select=document.getElementById('pcPartnerType');if(select)select.innerHTML='<option value="">All types</option>'+state.petconnect.types.map(function(type){return '<option value="'+type.id+'">'+escHtml(type.label)+'</option>';}).join('');}var allSelected=state.petconnect.partners.length&&state.petconnect.partners.every(function(partner){return state.petconnect.partnerSelection[partner.id];});pcTable('petconnectPartners',['<input type="checkbox" '+(allSelected?'checked':'')+' onchange="toggleAllPetConnectPartners(this.checked)" title="Select this page">','Organization','Type','Address','Geo location','Invitation','Active','Actions'],state.petconnect.partners.map(function(partner){var invite=partner.is_verified?'Claimed':partner.invitation_sent_at?'Invited':'Uninvited';return '<tr><td><input type="checkbox" class="pc-partner-select" '+(state.petconnect.partnerSelection[partner.id]?'checked':'')+' onchange="togglePetConnectPartnerSelection('+partner.id+',this.checked)"></td><td><strong>'+escHtml(partner.company_name)+'</strong><br>'+escHtml(partner.email)+'</td><td>'+escHtml(partner.type_label)+'</td><td>'+escHtml([partner.address_line,partner.city,partner.state,partner.postal_code,partner.country].filter(Boolean).join(', '))+'</td><td>'+pcPartnerGeo(partner)+'</td><td>'+invite+'</td><td><input type="checkbox" '+(partner.is_active?'checked':'')+' onchange="togglePetConnectPartner('+partner.id+',this.checked)"></td><td><button class="btn-outline" onclick="editPetConnectPartner('+partner.id+')"><i class="fas fa-edit"></i></button> <button class="btn-danger" onclick="deletePetConnectPartner('+partner.id+')"><i class="fas fa-trash"></i></button></td></tr>'; }));renderPetConnectPartnerPagination();updatePartnerSelectionStatus(); } catch(e){showToast(e.message,'error');} }
function downloadPartnerCsvTemplate(){ var blob=new Blob(['organization_name,contact_name,email,phone,address_line,city,state,postal_code,country,organization_type,website\n'],{type:'text/csv'}); var link=document.createElement('a');link.href=URL.createObjectURL(blob);link.download='petconnect-organizations.csv';link.click();URL.revokeObjectURL(link.href); }
async function previewPartnerCsv(){var input=document.getElementById('partnerCsvFile');if(!input.files[0])return showToast('Choose a CSV file first','error');var form=new FormData();form.append('file',input.files[0]);try{var r=await fetch('/api/admin/petconnect/partners/csv-preview',{...creds,method:'POST',body:form}),d=await r.json();if(!r.ok)return showToast(d.error||'Could not review CSV','error');var summary=d.summary||{};document.getElementById('partnerCsvReview').innerHTML='<div class="csv-review"><p><strong>'+summary.total+' rows found.</strong> '+summary.valid+' valid, '+summary.invalid+' skipped. Showing the first '+summary.shown+' rows.</p>'+d.rows.map(function(row){return '<div>Row '+row.row+' '+escHtml(row.data.organization_name||'')+(row.errors.length?' — '+escHtml(row.errors.join('; ')):'')+'</div>';}).join('')+'<button class="btn-primary-sm" onclick="importPartnerCsv()">Import all valid organizations</button></div>';}catch(error){showToast('Could not review CSV. Please try again.','error');}}
async function importPartnerCsv(){var input=document.getElementById('partnerCsvFile');if(!input.files[0])return showToast('Choose a CSV file first','error');var form=new FormData();form.append('file',input.files[0]);var button=document.querySelector('#partnerCsvReview button.btn-primary-sm');if(button){button.disabled=true;button.textContent='Importing organizations...';}try{var r=await fetch('/api/admin/petconnect/partners/csv-import',{...creds,method:'POST',body:form}),d=await r.json();if(!r.ok)return showToast(d.error||'Could not import CSV','error');showToast(d.inserted+' organizations imported'+(d.skipped?' ('+d.skipped+' duplicates skipped)':''),'success');document.getElementById('partnerCsvReview').innerHTML='';loadPetConnectPartners();}catch(error){showToast('Could not import CSV. Please try again.','error');if(button){button.disabled=false;button.textContent='Import all valid organizations';}}}
async function retryPartnerGeocoding(id){state.petconnect.partnerSelection[id]=true;return retrySelectedPartnerGeocoding();}
async function retrySelectedPartnerGeocoding(){var ids=selectedPetConnectPartnerIds();if(!ids.length)return showToast('Select organizations to geocode','error');var r=await fetch('/api/admin/petconnect/partners/geocode-retry',{...creds,method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({partner_ids:ids})}),d=await r.json();if(!r.ok)return showToast(d.error||'Could not queue geocoding','error');state.petconnect.partnerSelection={};showToast(d.queued+' organizations queued for geocoding','success');loadPetConnectPartners();}
async function inviteSelectedPartners(){var ids=selectedPetConnectPartnerIds(),button=document.getElementById('inviteSelectedPartnersButton'),progress=document.getElementById('partnerInviteProgress'),text=document.getElementById('partnerInviteProgressText');if(!ids.length)return showToast('Select organizations to invite','error');button.disabled=true;button.innerHTML='<i class="fas fa-spinner fa-spin"></i> Sending...';progress.hidden=false;text.textContent='Sending '+ids.length+' invitation'+(ids.length===1?'':'s')+'...';try{var r=await fetch('/api/admin/petconnect/partners/invite',{...creds,method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({partner_ids:ids})}),raw=await r.text(),d;try{d=JSON.parse(raw);}catch(parseError){throw new Error('Server returned an unexpected response. Please try again after deployment finishes.');}if(!r.ok)throw new Error(d.error||'Could not send invitations');var summary=d.invited+' invitation'+(d.invited===1?'':'s')+' sent successfully.'+(d.failed?' '+d.failed+' failed'+(d.delivery_error?' ('+d.delivery_error+').':'.'):'')+(d.skipped?' '+d.skipped+' skipped because they are already active.':'');text.textContent=summary;showToast(summary,d.failed?'error':'success');if(d.invited)state.petconnect.partnerSelection={};loadPetConnectPartners();}catch(error){text.textContent='Invitation send failed: '+error.message;showToast(error.message,'error');}finally{button.disabled=false;button.textContent='Invite selected';}}
...
async function loadEmailHealth() { var target=document.getElementById('emailHealthStatus'); if(!target)return; try { var response=await fetch('/api/admin/email-health',creds), data=await response.json(); target.textContent=data.success?'SMTP status: connected and authenticated.':'SMTP status: '+(data.error||'SMTP_CONNECTION_FAILED'); target.style.color=data.success?'#3d8b55':'#c4622d'; } catch(error) { target.textContent='SMTP status: unable to run diagnostic.'; target.style.color='#c4622d'; } }
...
function renderPetConnect() {
  ...
  pcTable('petconnectMembers',['Member','Location','Verified','Email alerts','Actions'],pc.members.map(function(m){return '<tr><td><strong>'+escHtml(m.first_name+' '+m.last_name)+'</strong><br>'+escHtml(m.email)+'</td><td>'+escHtml([m.city,m.state,m.country].filter(Boolean).join(', ')||'—')+'</td><td>'+ (m.is_verified?'Yes':'No')+'</td><td><label><input type="checkbox" '+(m.email_alerts?'checked':'')+' onchange="savePetConnectMember('+m.id+',this.checked,'+(m.email_alert_radius||100)+')"> '+escHtml(m.email_alert_radius||100)+' mi</label></td><td class="col-actions"><button class="btn-outline" onclick="editPetConnectMember('+m.id+')" title="Edit"><i class="fas fa-edit"></i></button> <button class="btn-danger" onclick="deletePetConnectMember('+m.id+')" title="Delete"><i class="fas fa-trash"></i></button></td></tr>'; }));
...
function editPetConnectMember(id) { var m=state.petconnect.members.find(function(item){return item.id===id;}); if(!m)return; openModal('Edit PetConnect Member','<div class="form-row"><div class="form-group"><label>First name</label><input id="pmFirst" class="field-input" value="'+escHtml(m.first_name)+'"></div><div class="form-group"><label>Last name</label><input id="pmLast" class="field-input" value="'+escHtml(m.last_name)+'"></div><div class="form-group"><label>Email</label><input id="pmEmail" type="email" class="field-input" value="'+escHtml(m.email)+'"></div><div class="form-group"><label>Phone</label><input id="pmPhone" class="field-input" value="'+escHtml(m.phone||'')+'"></div><div class="form-group"><label>Street address</label><input id="pmAddress" class="field-input" value="'+escHtml(m.address_line||'')+'"></div><div class="form-group"><label>City</label><input id="pmCity" class="field-input" value="'+escHtml(m.city||'')+'"></div><div class="form-group"><label>State / Province</label><input id="pmState" class="field-input" value="'+escHtml(m.state||'')+'"></div><div class="form-group"><label>Country</label><select id="pmCountry" class="field-input"><option value="US" '+(m.country==='US'?'selected':'')+'>United States</option><option value="CA" '+(m.country==='CA'?'selected':'')+'>Canada</option></select></div><div class="form-group"><label>Postal code</label><input id="pmPostal" class="field-input" value="'+escHtml(m.postal_code||'')+'"></div><div class="form-group"><label>Alert radius (miles)</label><input id="pmRadius" type="number" min="1" max="1000" class="field-input" value="'+(m.email_alert_radius||100)+'"></div><label class="checkbox-label"><input id="pmAlerts" type="checkbox" '+(m.email_alerts?'checked':'')+'> Receive email alerts</label></div><div class="form-actions"><button class="btn-outline" onclick="closeModal()">Cancel</button><button class="btn-primary-sm" onclick="saveEditedPetConnectMember('+m.id+')">Save changes</button></div>'); }
async function saveEditedPetConnectMember(id) { var payload={first_name:document.getElementById('pmFirst').value,last_name:document.getElementById('pmLast').value,email:document.getElementById('pmEmail').value,phone:document.getElementById('pmPhone').value,address_line:document.getElementById('pmAddress').value,city:document.getElementById('pmCity').value,state:document.getElementById('pmState').value,country:document.getElementById('pmCountry').value,postal_code:document.getElementById('pmPostal').value,email_alert_radius:document.getElementById('pmRadius').value,email_alerts:document.getElementById('pmAlerts').checked}; var r=await fetch('/api/admin/petconnect/members/'+id,{...creds,method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}),d=await r.json(); if(!r.ok){showToast(d.error||'Could not update member','error');return;} closeModal(); loadPetConnectMembers(); showToast('Member updated','success'); }
async function deletePetConnectMember(id) { if(!confirm('Delete this member and all registered pets and alerts?'))return; var r=await fetch('/api/admin/petconnect/members/'+id,{...creds,method:'DELETE'}),d=await r.json(); if(!r.ok){showToast(d.error||'Could not delete member','error');return;} loadPetConnect(); showToast('Member deleted','success'); }
...
async function togglePetConnectPartner(id,active) { var partner=state.petconnect.partners.find(function(p){return p.id===id;}); if(!partner)return; await fetch('/api/admin/petconnect/partners/'+id,{...creds,method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({...partner,is_active:active})}); showToast('Organization updated','success'); loadPetConnectPartners(); }
async function deletePetConnectPartner(id) { if(!confirm('Delete this organization?'))return; await fetch('/api/admin/petconnect/partners/'+id,{...creds,method:'DELETE'}); loadPetConnectPartners(); }
function editPetConnectPartner(id) { var p=state.petconnect.partners.find(function(item){return item.id===id;}); if(!p)return; var types=state.petconnect.types||[]; openModal('Edit PetConnect Organization','<div class="form-row"><div class="form-group"><label>Type</label><select id="pcEditType" class="field-input">'+types.map(function(t){return '<option value="'+t.id+'" '+(String(t.id)===String(p.partner_type_id)?'selected':'')+'>'+escHtml(t.label)+'</option>';}).join('')+'</select></div><div class="form-group"><label>Organization</label><input id="pcEditCompany" class="field-input" value="'+escHtml(p.company_name)+'"></div><div class="form-group"><label>Contact</label><input id="pcEditContact" class="field-input" value="'+escHtml(p.contact_name)+'"></div><div class="form-group"><label>Email</label><input id="pcEditEmail" type="email" class="field-input" value="'+escHtml(p.email)+'"></div><div class="form-group"><label>Phone</label><input id="pcEditPhone" class="field-input" value="'+escHtml(p.phone||'')+'"></div><div class="form-group"><label>Street address</label><input id="pcEditAddress" class="field-input" value="'+escHtml(p.address_line||'')+'"></div><div class="form-group"><label>City</label><input id="pcEditCity" class="field-input" value="'+escHtml(p.city||'')+'"></div><div class="form-group"><label>State / Province</label><input id="pcEditState" class="field-input" value="'+escHtml(p.state||'')+'"></div><div class="form-group"><label>Postal code</label><input id="pcEditPostal" class="field-input" value="'+escHtml(p.postal_code||'')+'"></div><div class="form-group"><label>Country</label><select id="pcEditCountry" class="field-input"><option value="US" '+(p.country==='US'?'selected':'')+'>United States</option><option value="CA" '+(p.country==='CA'?'selected':'')+'>Canada</option></select></div><div class="form-group"><label>Website</label><input id="pcEditWebsite" class="field-input" value="'+escHtml(p.website||'')+'"></div></div><div class="form-actions"><button class="btn-outline" onclick="closeModal()">Cancel</button><button class="btn-primary-sm" onclick="saveEditedPetConnectPartner('+id+')">Save changes</button></div>'); }
async function saveEditedPetConnectPartner(id) { var current=state.petconnect.partners.find(function(item){return item.id===id;})||{}; var payload={partner_type_id:document.getElementById('pcEditType').value,company_name:document.getElementById('pcEditCompany').value,contact_name:document.getElementById('pcEditContact').value,email:document.getElementById('pcEditEmail').value,phone:document.getElementById('pcEditPhone').value,address_line:document.getElementById('pcEditAddress').value,city:document.getElementById('pcEditCity').value,state:document.getElementById('pcEditState').value,postal_code:document.getElementById('pcEditPostal').value,country:document.getElementById('pcEditCountry').value,website:document.getElementById('pcEditWebsite').value,is_active:!!current.is_active}; var r=await fetch('/api/admin/petconnect/partners/'+id,{...creds,method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}),d=await r.json();if(!r.ok)return showToast(d.error||'Could not update organization','error');closeModal();loadPetConnectPartners();showToast('Organization updated','success');}
function newPetConnectPartner() { var types=state.petconnect.types||[]; var options=types.map(function(t){return '<option value="'+t.id+'">'+escHtml(t.label)+'</option>';}).join(''); openModal('Add PetConnect Organization','<div class="form-row"><div class="form-group"><label>Type</label><select id="pcType" class="field-input">'+options+'</select></div><div class="form-group"><label>Organization</label><input id="pcCompany" class="field-input"></div><div class="form-group"><label>Contact</label><input id="pcContact" class="field-input"></div><div class="form-group"><label>Email</label><input id="pcEmail" type="email" class="field-input"></div><div class="form-group"><label>Phone</label><input id="pcPhone" class="field-input"></div><div class="form-group"><label>Street address</label><input id="pcAddress" class="field-input"></div><div class="form-group"><label>City</label><input id="pcCity" class="field-input"></div><div class="form-group"><label>State / Province</label><input id="pcState" class="field-input"></div><div class="form-group"><label>Postal code</label><input id="pcPostal" class="field-input"></div><div class="form-group"><label>Country</label><select id="pcCountry" class="field-input"><option value="US">United States</option><option value="CA">Canada</option></select></div><div class="form-group"><label>Website</label><input id="pcWebsite" class="field-input"></div></div><div class="form-actions"><button class="btn-outline" onclick="closeModal()">Cancel</button><button class="btn-primary-sm" onclick="savePetConnectPartner()">Save organization</button></div>'); }
async function savePetConnectPartner() { var payload={partner_type_id:document.getElementById('pcType').value,company_name:document.getElementById('pcCompany').value,contact_name:document.getElementById('pcContact').value,email:document.getElementById('pcEmail').value,phone:document.getElementById('pcPhone').value,address_line:document.getElementById('pcAddress').value,city:document.getElementById('pcCity').value,state:document.getElementById('pcState').value,postal_code:document.getElementById('pcPostal').value,country:document.getElementById('pcCountry').value,website:document.getElementById('pcWebsite').value}; var r=await fetch('/api/admin/petconnect/partners',{...creds,method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});var d=await r.json();if(!r.ok){showToast(d.error||'Could not add organization','error');return;}closeModal();loadPetConnectPartners();showToast('Organization added','success');}
async function testEmailDelivery() { var email=window.prompt('Send a test email to:'); if(!email)return; try { var health=await fetch('/api/admin/email-health',creds); var healthData=await health.json(); if(!healthData.success)throw new Error(healthData.error||'SMTP_CONNECTION_FAILED'); var response=await fetch('/api/admin/email-test',{...creds,method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:email})}); var data=await response.json(); if(!response.ok)throw new Error(data.error||'SMTP_SEND_FAILED'); showToast(data.message,'success'); } catch(error) { showToast('Email test failed: '+error.message,'error'); } }
