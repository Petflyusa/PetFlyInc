...
async function previewPartnerCsv(){ var input=document.getElementById('partnerCsvFile'); if(!input.files[0])return showToast('Choose a CSV file first','error'); var form=new FormData();form.append('file',input.files[0]);var r=await fetch('/api/admin/petconnect/partners/csv-preview',{...creds,method:'POST',body:form}),d=await r.json();if(!r.ok)return showToast(d.error||'Could not review CSV','error');document.getElementById('partnerCsvReview').innerHTML='<div class="csv-review"><p>Review '+d.rows.length+' rows, then import valid selections.</p>'+d.rows.map(function(row){return '<label><input type="checkbox" class="pc-csv-row" '+(!row.errors.length?'checked':'disabled')+' data-row="'+row.row+'"> Row '+row.row+' '+escHtml(row.data.organization_name||'')+(row.errors.length?' — '+escHtml(row.errors.join('; ')):'')+'</label>';}).join('')+'<button class="btn-primary-sm" onclick="importPartnerCsv()">Import selected valid rows</button></div>';window.pcCsvRows=d.rows;}
async function importPartnerCsv(){var selected=Array.from(document.querySelectorAll('.pc-csv-row:checked')).map(function(el){return window.pcCsvRows.find(function(row){return String(row.row)===el.dataset.row;}).data;});if(!selected.length)return showToast('Select at least one valid row','error');var button=document.querySelector('#partnerCsvReview button.btn-primary-sm');if(button){button.disabled=true;button.textContent='Importing '+selected.length+' organizations...';}try{var r=await fetch('/api/admin/petconnect/partners/csv-import',{...creds,method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({rows:selected})}),d=await r.json();if(!r.ok)return showToast(d.error||'Could not import CSV','error');showToast(d.inserted+' organizations imported','success');document.getElementById('partnerCsvReview').innerHTML='';loadPetConnectPartners();}catch(error){showToast('Could not import CSV. Please try again.','error');if(button){button.disabled=false;button.textContent='Import selected valid rows';}}}
...
async function inviteSelectedPartners(){var ids=selectedPetConnectPartnerIds();if(!ids.length)return showToast('Select organizations to invite','error');var r=await fetch('/api/admin/petconnect/partners/invite',{...creds,method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({partner_ids:ids})}),d=await r.json();if(!r.ok)return showToast(d.error||'Could not send invitations','error');showToast(d.invited+' invitations sent','success');loadPetConnectPartners();}
