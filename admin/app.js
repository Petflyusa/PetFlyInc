...
async function loadPetConnectPartners() { try { var q=new URLSearchParams({search:pcValue('pcPartnerSearch'),partner_type_id:pcValue('pcPartnerType'),active:pcValue('pcPartnerActive'),verified:pcValue('pcPartnerVerified'),country:pcValue('pcPartnerCountry'),invite:pcValue('pcPartnerInvite')}); var d=await pcJson('/api/admin/petconnect/partners?'+q); state.petconnect.partners=d.partners||[]; if(!state.petconnect.types.length){var t=await pcJson('/api/admin/petconnect/partner-types');state.petconnect.types=t.types||[];var select=document.getElementById('pcPartnerType');if(select)select.innerHTML='<option value="">All types</option>'+state.petconnect.types.map(function(type){return '<option value="'+type.id+'">'+escHtml(type.label)+'</option>';}).join('');} pcTable('petconnectPartners',['Select','Organization','Type','Address','Invitation','Active','Actions'],state.petconnect.partners.map(function(p){var invite=p.is_verified?'Claimed':p.invitation_sent_at?'Invited':'Uninvited';return '<tr><td><input type="checkbox" class="pc-partner-select" value="'+p.id+'"></td><td><strong>'+escHtml(p.company_name)+'</strong><br>'+escHtml(p.email)+'</td><td>'+escHtml(p.type_label)+'</td><td>'+escHtml([p.address_line,p.city,p.state,p.postal_code,p.country].filter(Boolean).join(', '))+'</td><td>'+invite+'</td><td><input type="checkbox" '+(p.is_active?'checked':'')+' onchange="togglePetConnectPartner('+p.id+',this.checked)"></td><td><button class="btn-outline" onclick="editPetConnectPartner('+p.id+')"><i class="fas fa-edit"></i></button> <button class="btn-danger" onclick="deletePetConnectPartner('+p.id+')"><i class="fas fa-trash"></i></button></td></tr>'; })); } catch(e){showToast(e.message,'error');} }
