...
async function populateContractCountries() { try { var d=await (await fetch('/api/countries')).json(); state.countries=d.countries||[]; document.querySelectorAll('.contract-country').forEach(function(select) { var selected=select.value; Array.from(select.options).forEach(function(option) { if (option.value) option.remove(); }); state.countries.forEach(function(country) { var option=document.createElement('option'); option.value=country.country_name; option.textContent=country.country_name; option.selected=country.country_name===selected; select.appendChild(option); }); }); } catch(e){console.error(e);} }
